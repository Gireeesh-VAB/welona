...
import { useAdminOffers, useCreateAdminOffer, useUpdateAdminOffer } from '@/hooks/useAdminCustomers';
...
export default function OffersTab({ customerId }: { customerId: string }) {
  ...
  const { data, isLoading } = useAdminOffers(customerId);
  const createOffer = useCreateAdminOffer(customerId);
  const updateOffer = useUpdateAdminOffer(customerId);
