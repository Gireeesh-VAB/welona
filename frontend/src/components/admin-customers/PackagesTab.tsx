...
import { useAdminTreatments } from '@/hooks/useAdminSales';
import { ApiClientError } from '@/lib/api-client';
import { PACKAGE_STATUSES } from '@shared/enums';
import { formatDate, formatMoney, titleCase, toMinorUnits } from '@shared/format';
import type { Package } from '@shared/types/customer-modules';

const { Text } = Typography;
const statusOptions = PACKAGE_STATUSES.map((s) => ({ label: titleCase(s), value: s }));

/** Packages module (admin) — session-based treatment packages for a customer. */
export default function PackagesTab({ customerId }: { customerId: string }) {
  const { message } = App.useApp();
  const { data, isLoading } = useAdminPackages(customerId);
  const { data: treatments } = useAdminTreatments();
  const createPackage = useCreateAdminPackage(customerId);
  const updatePackage = useUpdateAdminPackage(customerId);
