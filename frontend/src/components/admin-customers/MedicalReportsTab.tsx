...
import { useAdminMedicalReports, useCreateAdminMedicalReport } from '@/hooks/useAdminCustomers';
...
export default function MedicalReportsTab({ customerId }: { customerId: string }) {
  ...
  const { data, isLoading } = useAdminMedicalReports(customerId);
  const createReport = useCreateAdminMedicalReport(customerId);
