...
import {
  useAdminQuotations,
  useAdminQuotation,
  useCreateAdminQuotation,
  useUpdateAdminQuotation,
  useAdminQuotationAction,
  useAdminSalespeople,
...
import CustomerPicker from '@/components/admin-sales/CustomerPicker';
...
function QuotationDrawer({ id, onClose }: { id: string | null; onClose: () => void }) {
  ...
  const { data: quotation, isLoading } = useAdminQuotation(id);
  const action = useAdminQuotationAction();
  const updateQuotation = useUpdateAdminQuotation();
...
export default function QuotationsPage() {
  ...
  const { data, isLoading } = useAdminQuotations({ page, limit: 10, status: statusFilter });

  const { data: salespeople } = useAdminSalespeople();
  const createQuotation = useCreateAdminQuotation();
