...
import {
  ...
  useAdminOrderStatus,
  useUpdateAdminOrder,
  useCreateAdminDelivery,
  useAdminDeliveryAction,
...
import type { Delivery, Invoice, LineItem, SalesOrder } from '@shared/types/sales';
...
function OrderDrawer({ id, onClose }: { id: string | null; onClose: () => void }) {
  ...
  const orderStatus = useAdminOrderStatus();
  const updateOrder = useUpdateAdminOrder();
  const createDelivery = useCreateAdminDelivery();
  const deliveryAction = useAdminDeliveryAction();
...
export default function OrdersPage() {
  ...
  const { data, isLoading } = useAdminOrders({ page, limit: 10, status: statusFilter });
