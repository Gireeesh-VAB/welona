...
import {
  ...
  useUpdateAdminLead,
  useConvertAdminLead,
  useConvertAdminLeadToCustomer,
  useAdminFollowUps,
  useCreateAdminFollowUp,
  useAdminSalespeople,
...
export default function EnquiryDetailPage() {
  ...
  const { data: followUps } = useAdminFollowUps(id);
  const { data: salespeople } = useAdminSalespeople();

  const leadStatus = useAdminLeadStatus();
  const updateLead = useUpdateAdminLead();
  const convertLead = useConvertAdminLead();
  const convertToCustomer = useConvertAdminLeadToCustomer();
  ...
  return (
    <div>
      ...
      <Button
        ...
        onClick={() => router.push('/admin/sales/enquiries')}
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <Space style={{ marginTop: 16, flexWrap: 'wrap' }}>
          ...
          {lead.customerId && (
            <Button type="link" onClick={() => router.push(`/admin/customers/${lead.customerId}`)}>
