...
export default function EnquiriesPage() {
  ...
  const { data, isLoading } = useAdminLeads({ page, limit: 10, status: statusFilter });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setModalOpen(false);
      form.resetFields();
      router.push(`/admin/sales/enquiries/${lead.id}/followups`);
    } catch (e) {
  ...
  return (
    <div>
      ...
      <Card>
        <Space style={{ marginBottom: 16 }}>
          <Select
            ...
            options={['new', 'contacted', 'qualified', 'unqualified', 'converted', 'lost'].map(
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} layout="vertical" preserve={false} initialValues={{ leadTransfer: false }}>
          <Form.Item
            name="contactPhone"
            label="Mobile No"
          ...
          >
            ...
            <Form.Item
              name="branchId"
              label="Branch"
              rules={[{ required: true, message: 'Assign the enquiry to a branch' }]}
            >
              <Select
                showSearch
