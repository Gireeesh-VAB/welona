...
export default function AdminSuppliersPage() {
  ...
  const navItem = getAdminNavItem('procurement-suppliers')!;
  ...
  const openEdit = (s: AdminSupplier) => {
    ...
    setProductMappings(
      (s.products ?? []).map((p) => ({
        ...
        unitPrice: p.unitPrice != null ? p.unitPrice / 100 : null, // paise â†’ â‚¹
  ...
  const onSubmit = async () => {
    ...
    const body: AdminSupplierCreateInput = {
      ...
      products: productMappings
        ...
        .map((p) => ({
          productId: p.productId,
          unitPrice: p.unitPrice != null ? Math.round(p.unitPrice * 100) : undefined, // â‚¹ â†’ paise
  ...
  const emptyCell = <Text style={{ color: colors.text.placeholder, fontSize: 12 }}>â€”</Text>;
  ...
  return (
    <div>
        ...
        <Table<AdminSupplier>
          ...
          onRow={(row) => ({
            onClick: (e) => {
              ...
              router.push(`/admin/procurement/suppliers/${row.id}`);
            },
      ...
      <Modal
        title={editing ? `Edit Supplier â€” ${editing.name}` : 'Add Supplier'}
        open={modalOpen}
      ...
      >
        ...
        {productMappings.length > 0 && (
          <div style={{ display: 'flex', gap: 6, marginBottom: 4, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ width: 120, fontSize: 11, color: colors.text.placeholder, paddingLeft: 2 }}>Unit Price (â‚¹)</div>
        ...
        {productMappings.map((row, idx) => (
          <div key={idx} style={{ display: 'flex', gap: 6, marginBottom: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <Select
              ...
              notFoundContent={productOptsLoading ? 'Loadingâ€¦' : 'No products found'}
            />
            <InputNumber
              placeholder="0.00"
              min={0}
