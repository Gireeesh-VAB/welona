...
import {
  PHONE_ENQUIRY_ROWS,
  type PhoneEnquiryRow,
  type PhoneEnquiryOutcome,
} from '@/lib/sample-data/phone-enquiry';

const { Title, Text } = Typography;
const { RangePicker } = DatePicker;

type ReportFormat = 'detailed' | 'summary_outcome' | 'summary_branch' | 'summary_source' | 'summary_received';
...
function summarise(
  rows: PhoneEnquiryRow[],
  by: 'outcome' | 'branchName' | 'source' | 'receivedBy',
): SummaryRow[] {
  const map = new Map<string, SummaryRow>();
  for (const r of rows) {
    const group = r[by];
    const existing = map.get(group) ?? {
      key: group, group, calls: 0, booked: 0, leadCreated: 0,
      followUp: 0, lost: 0, durationMins: 0, conversionPct: 0,
    };
    existing.calls += 1;
    if (r.outcome === 'Booked') existing.booked += 1;
    else if (r.outcome === 'Lead Created') existing.leadCreated += 1;
  ...
  return Array.from(map.values()).map((s) => ({
    ...s,
    conversionPct: s.calls === 0 ? 0 : Math.round((s.booked / s.calls) * 100),
...
export default function AdminPhoneEnquiryPage() {
  const colors = useBrandColors();
  const { isBranchSession, lockedBranchId } = useBranchLock();
  const navItem = getAdminNavItem('report-cm-phone-enquiry')!;

  const [dateRange, setDateRange] = useState<[Dayjs, Dayjs] | null>(null);
  const [branchId, setBranchId] = useState<string | undefined>(undefined);
  const [category, setCategory] = useState<string | undefined>(undefined);
  const [outcome, setOutcome] = useState<PhoneEnquiryOutcome | undefined>(undefined);
  ...
  const filteredRows = useMemo(() => {
    ...
    return PHONE_ENQUIRY_ROWS.filter((r) => {
  ...
  const outcomeColor = (o: PhoneEnquiryOutcome): string => {
    if (o === 'Booked') return colors.status.success;
    if (o === 'Lead Created') return colors.status.info;
  ...
  const handleExport = () => {
    if (format === 'detailed') {
      const headers = ['S No','Call Date','Call Time','Branch','Caller','Mobile','Treatment Interest','Category','Source','Received By','Duration (mins)','Outcome','Remarks'];
      const values = filteredRows.map((r, i) => [
        i + 1, dayjs(r.callAt).format('DD-MM-YYYY'), dayjs(r.callAt).format('HH:mm'),
        r.branchName, r.callerName, r.mobileNumber, r.treatmentInterest, r.category,
        r.source, r.receivedBy, r.durationMins, r.outcome, r.remarks,
      ]);
      downloadCsv(`phone-enquiry-${dayjs().format('YYYY-MM-DD')}.csv`, rowsToCsv(headers, values));
      return;
    }
    const by = format === 'summary_branch' ? 'branchName'
      : format === 'summary_source' ? 'source'
      : format === 'summary_received' ? 'receivedBy'
      : 'outcome';
    const groupLabel = format === 'summary_branch' ? 'Branch'
      : format === 'summary_source' ? 'Source'
      : format === 'summary_received' ? 'Received By'
      : 'Outcome';
    const summary = summarise(filteredRows, by);
    const headers = [groupLabel, 'Calls', 'Booked', 'Lead Created', 'Follow-Up', 'Lost', 'Duration (mins)', 'Conversion %'];
    const values = summary.map((s) => [s.group, s.calls, s.booked, s.leadCreated, s.followUp, s.lost, s.durationMins, s.conversionPct]);
    downloadCsv(`phone-enquiry-${format}-${dayjs().format('YYYY-MM-DD')}.csv`, rowsToCsv(headers, values));
  };
  ...
  const detailedColumns: ColumnsType<PhoneEnquiryRow> = useMemo(() => [
    {
      title: 'S No', key: 'sno', width: 70, fixed: 'left',
      render: (_: unknown, __: PhoneEnquiryRow, idx: number) => (page - 1) * limit + idx + 1,
    },
    {
      title: 'Call Date & Time', dataIndex: 'callAt', width: 180, fixed: 'left',
    ...
    {
      title: 'Caller', dataIndex: 'callerName', width: 200,
      sorter: (a, b) => a.callerName.localeCompare(b.callerName),
      render: (v: string, row) => (
        <div>
    ...
    { title: 'Received By', dataIndex: 'receivedBy', width: 170,
  ...
  const summaryColumns: ColumnsType<SummaryRow> = useMemo(() => {
    ...
    return [
      ...
      { title: 'Lead Created', dataIndex: 'leadCreated', width: 130, align: 'left',
  ...
  const totals = useMemo(() => filteredRows.reduce((acc, r) => ({
    ...
    leadCreated: acc.leadCreated + (r.outcome === 'Lead Created' ? 1 : 0),
    followUp: acc.followUp + (r.outcome === 'Follow-Up' ? 1 : 0),
    lost: acc.lost + (r.outcome === 'Lost' ? 1 : 0),
    duration: acc.duration + r.durationMins,
  }), { booked: 0, leadCreated: 0, followUp: 0, lost: 0, duration: 0 }), [filteredRows]);
  ...
  const summaryRows = useMemo(() => {
    ...
    if (format === 'summary_received') return summarise(filteredRows, 'receivedBy');
        ...
        <Row gutter={[12, 12]} justify="start">
          ...
          <Col xs={24} sm={12} md={8} lg={6} xl={5}>
            ...
            <Select style={{ width: '100%', marginTop: 4 }} placeholder="All outcomes" value={outcome}
              onChange={(v: PhoneEnquiryOutcome | undefined) => { setOutcome(v); setPage(1); }}
              options={[
          ...
          <Col xs={24} sm={12} md={8} lg={6} xl={5}>
            ...
            <Select style={{ width: '100%', marginTop: 4 }} value={format}
              ...
              options={[
                ...
                { value: 'summary_received', label: 'Summary — by Received By' },
              ]} />
          </Col>
        </Row>
      </Card>
            ...
            summary={() => summaryRows.length > 0 ? (
              <Table.Summary fixed>
                <Table.Summary.Row style={{ background: colors.black.primary }}>
                  ...
                  <Table.Summary.Cell index={3} align="left">{totals.leadCreated > 0 ? <strong style={{ color: colors.status.info }}>{totals.leadCreated}</strong> : '—'}</Table.Summary.Cell>
                  ...
                  <Table.Summary.Cell index={6} align="left"><strong>{totals.duration} mins</strong></Table.Summary.Cell>
                  <Table.Summary.Cell index={7} align="left"><strong>{conversionRate}%</strong></Table.Summary.Cell>
                </Table.Summary.Row>
              </Table.Summary>
            ) : null} />
        ) : (
          <Table<PhoneEnquiryRow> rowKey="key" loading={branchesLoading || categoriesLoading}
            columns={detailedColumns} dataSource={filteredRows} pagination={pagination}
            size="middle" scroll={{ x: 2330 }}
            locale={{ emptyText: <Empty description="No phone enquiries match the selected filters" /> }}
