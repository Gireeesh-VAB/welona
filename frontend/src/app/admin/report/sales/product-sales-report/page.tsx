...
export default function AdminProductSalesReportPage() {
  ...
  const navItem = getAdminNavItem('report-sales-product-sales')!;
  ...
  const handleExport = () => {
    if (format === 'detailed') {
      const headers = [
        ...
        'Package Amount', 'Tax Amount', 'Cash', 'Credit Cards', 'Office Scan',
        'Cheque', 'Bajaj', 'EzFinanz', 'SaveIn', 'Shopse', 'Fibe',
      ...
      const values = filteredRows.map((r, i) => [
        ...
        r.mobileNumber, r.packageDetails, r.category,
      ...
      downloadCsv(`product-sales-${dayjs().format('YYYY-MM-DD')}.csv`, rowsToCsv(headers, values));
      return;
    }
    ...
    downloadCsv(
      `product-sales-${format}-${dayjs().format('YYYY-MM-DD')}.csv`,
  ...
  const detailedColumns: ColumnsType<SalesReportRow> = useMemo(
    () => [
      ...
      { title: 'Tax Amount', dataIndex: 'taxAmount', width: 120, align: 'left', render: moneyCell },
              ...
              filteredRows.length > 0 ? (
                <Table.Summary fixed>
                  <Table.Summary.Row style={{ background: colors.black.primary }}>
                    ...
                    <Table.Summary.Cell index={10} align="left">{formatMoney(totals.taxAmount)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={11} align="left">{formatMoney(totals.cash)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={12} align="left">{formatMoney(totals.creditCards)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={13} align="left">{formatMoney(totals.officeScan)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={14} align="left">{formatMoney(totals.cheque)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={15} align="left">{formatMoney(totals.bajaj)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={16} align="left">{formatMoney(totals.ezFinanz)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={17} align="left">{formatMoney(totals.saveIn)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={18} align="left">{formatMoney(totals.shopse)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={19} align="left">{formatMoney(totals.fibe)}</Table.Summary.Cell>
                    <Table.Summary.Cell index={20} align="left">
                      <strong style={{ color: colors.status.success }}>{formatMoney(totals.paidAmount)}</strong>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={21} align="left">
                      <strong style={{ color: colors.status.warning }}>{formatMoney(totals.balanceAmount)}</strong>
                    </Table.Summary.Cell>
                  </Table.Summary.Row>
                </Table.Summary>
