...
import { quotationUpdateSchema } from '@/lib/sales/schemas';
...
/** GET /api/v1/sales/quotations/[id] — quotation detail with line items. */
export const GET = route<Ctx>(async (req, { params }) => {
  const claims = requireAuth(req);
  requirePermission(claims, 'sales:read');
  return ok(await loadQuotation(claims.orgId, params.id));
});

/**
 * PATCH /api/v1/sales/quotations/[id] — edit a quotation. Line items and
 * pricing can only change while the quotation is still a draft.
 */
export const PATCH = route<Ctx>(async (req, { params }) => {
  const claims = requireAuth(req);
  requirePermission(claims, 'sales:update');
  const existing = await loadQuotation(claims.orgId, params.id);
