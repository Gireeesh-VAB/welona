...
/**
 * GET /api/v1/admin/sales/dashboard — sales pipeline overview: KPI roll-ups,
  ...
  const [leads, quotations, orders, payments, invoices, staff, todayLeads, dueFollowups] =
    await Promise.all([
      db.lead.findMany({
        ...
        select: { status: true, ownerStaffId: true },
      }),
      ...
      db.invoice.findMany({
        where: { orgId, status: { not: 'void' }, ...branchFilter },
      ...
      db.lead.findMany({
        where: { orgId, createdAt: { gte: startOfToday }, ...branchFilter },
        include: { treatment: { select: { name: true } } },
        orderBy: { createdAt: 'desc' },
        take: 100,
      }),
      db.followUp.findMany({
        // FollowUp has no branchId — scope via the related lead's branch.
        where: {
          orgId,
          status: 'pending',
