...
export function toStockIndent(row: StockIndentWithRelations): StockIndent {
  return {
    ...
    dispatchMethod: row.dispatchMethod ?? null,
    vehicleNumber: row.vehicleNumber ?? null,
    driverName: row.driverName ?? null,
    driverMobile: row.driverMobile ?? null,
    pickupEmployeeId: row.pickupEmployeeId ?? null,
    providerName: row.providerName ?? null,
    trackingNumber: row.trackingNumber ?? null,
    deliveryCharges: row.deliveryCharges ?? null,
    customMethodName: row.customMethodName ?? null,
